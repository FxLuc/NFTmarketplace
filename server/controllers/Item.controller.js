...
const getItems = (req, res) => {
    Item.find().sort('-createdAt').limit(10).then(products => res.status(200).json(products))
}

const searchItem = (req, res) => {
    Item.find(({ name: { $regex: req.body.name, $options: 'i' }})).sort('-createdAt').limit(10).then( products => {
