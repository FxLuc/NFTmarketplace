...
(async () => {
  ...
  ItemManagerContract.events.ItemStateChanged().on('data', async event => {
    ...
    if (event.returnValues.state == '0' && event.returnValues.itemIndex == (lastItemIndex - 1)) createdItem(event)

    // listen Item sold event
    else if (event.returnValues.state == '1') soldItem(event)

    // listen Item delivered event
    else if (event.returnValues.state == '2') deliveredItem(event)

    // listen Item canceled event
    else if (event.returnValues.state == '3') canceledItem(event)
  })
})()

async function createdItem(event) {
...
async function deliveredItem(event) {
  ItemManagerContract.methods.items(event.returnValues.itemIndex).call().then(sItemStruct => {
    (async () => {
      // wait for node fully synced
      await new Promise(resolve => setTimeout(resolve, 5000))
...
async function canceledItem(event) {
  ItemManagerContract.methods.items(event.returnValues.itemIndex).call().then(sItemStruct => {
    (async () => {
      Item.findByIdAndUpdate(sItemStruct._item, {
        state: 3
